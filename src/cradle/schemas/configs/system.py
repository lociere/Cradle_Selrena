...
class PerceptionConfig(BaseModel):
    ...
    strict_wake_word: bool = Field(default=False, description="严格唤醒模式：仅包含唤醒词的输入才能进入意识流")
    wake_timeout_sec: float = Field(default=30.0, ge=1.0, le=600.0, description="非严格模式下的注意力保持时长（秒）")
...
class ModelManagerConfig(BaseModel):
    ...
    size_threshold_mb: int = Field(default=10, ge=1, le=10240, description="判断单文件是否为有效权重的最小大小（MB）")
...
class NapcatConfig(BaseModel):
    ...
    listen_port: int = Field(default=6101, ge=1, le=65535, description="本地监听端口")
