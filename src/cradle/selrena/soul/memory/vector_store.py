...
class MemoryVessel:
    ...
    def __init__(self):
        ...
        self.persist_dir = os.path.join(
            os.getcwd(), "data", "memory", "vector_store")
        self._embed_model = None
        self._is_ready = False

        # 添加开关功能
        self.memory_enabled = global_config.get_soul().memory.enabled
    ...
    def initialize(self):
        ...
        configured_path = cfg_memory.model_path or cfg_memory.hf_repo
        auto_download = cfg_memory.auto_download

        try:
            model_name_or_path = global_model_manager.resolve_model_path(
                configured_path, auto_download=auto_download)
        except Exception as e:
            self.logger.warning(
                f"[MemoryVessel] ModelManager 解析/下载失败 ({configured_path}): {e}; 将退回到远端标识加载")
            model_name_or_path = cfg_memory.hf_repo

        source = "local" if os.path.isdir(
            str(model_name_or_path)) else "remote"
        # 结构化日志：记录配置、决策与解析结果
        ...
        self.logger.info(
            f"[MemoryVessel] 加载 Embedding 模型: {os.path.basename(str(model_name_or_path))} (Force CPU) -> source: {source}")
        # 使用 ConfigManager 动态获取模型路径；若本地模型缺失权重，则回退到 HF 远程或尝试自动下载

        try:
            transformers_logging.set_verbosity_error()
            self._embed_model = SentenceTransformer(
                model_name_or_path, device="cpu")
    ...
    def _generate_embedding(self, text: str) -> List[float]:
        ...
        if not self._embed_model:
            self.logger.warning(
                "[MemoryVessel] ⚠️ 尝试在初始化前使用 Embedding 模型! 正触发自动初始化...")
            self.initialize()
