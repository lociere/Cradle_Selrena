...
from cradle.utils.string import clean_for_tts, extract_emotion_and_clean_text
...
class GPTSoVITSClient(BaseTTSClient):
    ...
    def _analyze_emotion(self, text: str) -> Tuple[str, str]:
        clean_text, detected = extract_emotion_and_clean_text(text)
    ...
    async def synthesize(self, text: str) -> Optional[Path]:
        ...
        if target_emo not in emotions_config and emotions_config:
            target_emo = list(emotions_config.keys())[
                0]  # Pick first available
        ...
        if not ref_wav_path.is_absolute():
            ref_wav_path = ProjectPath.PROJECT_ROOT / \
                base_path_str / emo_data["file"]  # Fix path join
        ...
        out_path = self._resolve_path(
            self.config.output_dir) / self._build_filename()
        ...
        try:
            ...
            if response.status_code == 200:
                logger.debug(
                    f"[TTS] GPT-SoVITS 返回内容长度: {len(response.content)} 字节")
                if not response.content or len(response.content) < 1024:
                    logger.error(
                        f"[TTS] GPT-SoVITS 返回内容为空或过小，未生成音频文件。Headers: {response.headers} Text: {response.text[:200]}")
                    return None
...
class UnifiedTTSClient:
    ...
    def switch_engine(self, engine_name: str):
        if engine_name in self.clients:
            ...
            logger.warning(
                f"[TTS] 未知引擎 {engine_name}，保持 {self.current_engine}")
