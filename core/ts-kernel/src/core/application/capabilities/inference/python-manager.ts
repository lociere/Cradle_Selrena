...
export class PythonAIManager {
  ...
  public async start(): Promise<void> {
    ...
    try {
      ...
      this._pythonProcess = spawn(venvPythonPath, [pythonEntryPath], {
        env: {
          ...process.env,
          SELRENA_IPC_BIND_ADDRESS: config.system.ipc.bind_address,
          SELRENA_CONFIG: JSON.stringify(config.persona),
          LOG_DIR: resolveLogDir(config.system.data_dir, config.system.log_dir),
          PYTHONUNBUFFERED: "1",
        },
  ...
  public async consolidateMemory(request: ConsolidateMemoryRequest): Promise<ConsolidateMemoryResponse> {
    if (!this._isReady) {
      throw new CoreException("Python AI层未就绪", ErrorCode.INFERENCE_ERROR);
    }
    ...
    const traceContext = createTraceContext();
    const ipcRequest = createIPCRequest(
      IPCMessageType.CONSOLIDATE_MEMORY,
      traceContext.trace_id,
      { ...request, trace_id: request.trace_id ?? traceContext.trace_id },
    );

    const response = await this.sendRequest(ipcRequest);
    if (!response.success) {
      logger.warn("记忆固化请求失败", { error: response.error?.message });
      throw new CoreException(`记忆固化失败: ${response.error?.message}`, ErrorCode.INFERENCE_ERROR);
    }

    return response.data as ConsolidateMemoryResponse;
  }
