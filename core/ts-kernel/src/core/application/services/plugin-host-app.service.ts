...
export class PluginHostAppService implements IPluginHostService {
  ...
  getConfig(): IPluginSystemConfig {
    ...
    return {
      system: { app_version: cfg.system.app_version },
      plugin: {
        plugin_root_dir: cfg.system.plugin.plugin_root_dir,
        sandbox: { timeout_ms: cfg.system.plugin.sandbox.timeout_ms },
      },
    };
  }
  ...
  registerCommand(
    ...
    if (!commandId.startsWith(`${pluginId}.`) && !commandId.startsWith(`${pluginId}:`)) {
      throw new Error(`插件 ${pluginId} 注册命令必须带命名空间前缀，当前为 ${commandId}`);
    }
    if (this._commands.has(commandId)) {
      throw new Error(`命令已存在: ${commandId}`);
    }

    this._commands.set(commandId, { pluginId, handler, metadata });
