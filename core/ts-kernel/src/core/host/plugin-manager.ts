...
type PluginManifestLite = {
  ...
  activationEvents: PluginManifest["activationEvents"];
...
export class PluginManager {
  ...
  public async loadPlugin(pluginId: string): Promise<void> {
    ...
    const manifestFromYaml = this.toManifestLite(manifestRaw, pluginId);

    if (manifestFromYaml.id !== pluginId) {
      throw new PluginException(`插件ID不匹配: ${pluginId} != ${manifestFromYaml.id}`, ErrorCode.PLUGIN_VALIDATION_FAILED);
    }
    ...
    if (!this.isVersionCompatible(appVersion, manifestFromYaml.minAppVersion)) {
      throw new PluginException(
        `插件最低版本不兼容: need=${manifestFromYaml.minAppVersion}, current=${appVersion}`,
        ErrorCode.PLUGIN_VALIDATION_FAILED
      );
    }

    const entryPath = path.resolve(pluginDir, manifestFromYaml.main);
    ...
    const resolvedExport = this.resolvePluginExport(pluginExports.default ?? pluginExports);
    const manifest = this.toManifestLite(
      resolvedExport.manifest ? { ...manifestRaw, ...resolvedExport.manifest } : manifestRaw,
      pluginId,
    );
    const plugin = resolvedExport.plugin;
    ...
    return {
      ...
      commands: {
        registerCommand: (commandId: string, handler, metadata?: ExtensionCommandMetadata) => {
          this.assertPluginPermission(pluginId, permissions, Permission.COMMAND_REGISTER, `注册命令 ${commandId}`);
          const disposable = this._hostService.registerCommand(pluginId, commandId, handler, metadata);
          subscriptions.push(disposable);
          return disposable;
        },
        executeCommand: async (commandId: string, ...args: unknown[]) => {
          this.assertPluginPermission(pluginId, permissions, Permission.COMMAND_EXECUTE, `执行命令 ${commandId}`);
          return this._hostService.executeCommand(commandId, ...args);
        },
        listCommands: async () => {
          return this._hostService.listCommands();
        },
      },
    };
  }

  private resolvePluginExport(exportedValue: PluginModuleExport): {
    plugin: SystemPlugin;
    manifest?: Partial<PluginManifest>;
  } {
    if (
      ...
      "plugin" in exportedValue &&
      exportedValue.plugin &&
      typeof exportedValue.plugin === "object"
    ) {
      return {
        plugin: exportedValue.plugin,
        manifest: exportedValue.manifest,
      };
    }

    return { plugin: exportedValue as SystemPlugin };
  }
