 ...
 *
 * 配置文件布局（configs/ 目录下）：
 *   app.yaml           → config.app      应用元信息
 *   ai.yaml            → config.ai       Python AI 层（人格 / 推理 / LLM）
 *   kernel.yaml        → config.kernel   TS 内核（IPC / 生命周期 / 插件）
...
export class ConfigManager {
  ...
  /**
   * 加载并校验全局配置。
   * 加载顺序：app.yaml → ai.yaml → kernel.yaml，合并后通过 Zod Schema 校验。
  ...
  public async init(): Promise<void> {
    ...
    try {
      // 加载三个配置域
      const appData = await this.loadYaml("app.yaml");
      const aiData = await this.loadYaml("ai.yaml");
      const kernelData = await this.loadYaml("kernel.yaml");

      // 从 secrets.yaml 注入 LLM API Key
      if (aiData?.llm) {
        await this.injectLLMApiKeysFromSecrets(aiData.llm);
      }
  ...
  public async generateDefaults(): Promise<void> {
    const targets: { relativePath: string; content: string }[] = [
      { relativePath: "app.yaml", content: APP_YAML_TEMPLATE },
      { relativePath: "ai.yaml", content: AI_YAML_TEMPLATE },
      { relativePath: "kernel.yaml", content: KERNEL_YAML_TEMPLATE },
      { relativePath: path.join("secret", "secrets.example.yaml"), content: SECRETS_EXAMPLE_YAML_TEMPLATE },
      { relativePath: path.join("plugin", "enabled-plugins.yaml"), content: ENABLED_PLUGINS_YAML_TEMPLATE },
    ];
  ...
  private async injectLLMApiKeysFromSecrets(llmConfig: {
    ...
    try {
      ...
      // secrets 解析失败时静默降级，继续使用 ai.yaml 中的配置
    }
  }
}
