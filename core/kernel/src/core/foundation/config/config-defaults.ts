...
/** app.yaml — 应用元信息与目录路径 */
export const APP_YAML_TEMPLATE = `# ╔════════════════════════════════════════════════════════════╗
# ║  Cradle Selrena — 应用配置 (app.yaml)                    ║
# ╚════════════════════════════════════════════════════════════╝
...
/** ai.yaml — Python AI 层全量配置 */
export const AI_YAML_TEMPLATE = `# ╔════════════════════════════════════════════════════════════╗
...
persona:
  ...
  base:
    ...
    gender: "女"                       # 性别
    appearance: ""                     # 外貌描述（用于系统提示词）
    background: ""                     # 角色背景故事

  # ── 核心性格 ──────────────────────────────────────────────
  core:
    personality: ""                    # 性格特征总述
    character_core: ""                 # 内在核心原则
    likes: ""                          # 兴趣偏好

  # ── 对话风格 ──────────────────────────────────────────────
  dialogue:
    dialogue_style: ""                 # 语言风格指导
    emotion_control: ""                # 情绪标签输出规则
...
/** kernel.yaml — TS 内核运行时配置 */
export const KERNEL_YAML_TEMPLATE = `# ╔════════════════════════════════════════════════════════════╗
# ║  Cradle Selrena — 内核配置 (kernel.yaml)                  ║
# ║  IPC 通信 / 模块生命周期 / 插件沙箱                       ║
# ╚════════════════════════════════════════════════════════════╝
...
plugin:
  plugin_root_dir: "plugins"           # 插件根目录（相对项目根）
  sandbox:
    enable_isolation: true             # 是否启用插件沙箱隔离
    timeout_ms: 5000                   # onActivate / onDeactivate 超时（ms）
    allow_native_modules: false        # 是否允许加载 native 模块
  default_permissions:                 # 新插件默认权限
    - CHAT_SEND
    - MEMORY_READ
    - MEMORY_WRITE
    - CONFIG_READ_SELF
    ...
    - CONFIG_READ_GLOBAL
    - EVENT_SUBSCRIBE
  plugin_blacklist: []                 # 插件黑名单（阻止加载的插件 ID）
`;
...
# LLM API Key（按提供商名称自动注入到 ai.yaml 中）
providers:
  deepseek:
    api_key: "sk-..."
  qwen:
...
/** plugin/enabled-plugins.yaml — 启用的插件列表 */
