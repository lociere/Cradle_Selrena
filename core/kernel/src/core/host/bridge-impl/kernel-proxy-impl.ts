...
import { PluginStateLogger } from "../../infrastructure/logger/plugin-state-logger";
import { DBManager } from "../../infrastructure/storage/db-manager";
import { AudioService } from "../../application/capabilities/audio/audio-service";
import { PluginSceneTranscriptService } from "../../application/capabilities/scene/plugin-scene-transcript-service";
import { AttentionSessionManager } from "../../domain/attention/attention-session-manager";
import { SceneRoutingManager } from "../../application/capabilities/scene/scene-routing-manager";
...
export class KernelProxyImpl implements IKernelProxy {
  ...
  async submitChannelMessage(request: PerceptionMessageRequest): Promise<ChatMessageResponse | null> {
  ...
  async getSelfConfig(): Promise<Record<string, any>> {
    ...
    const db = (await import("../../infrastructure/storage/db-manager")).DBManager.instance.getDB();
