...
export class EventBus {
  ...
  public async publish<T extends DomainEvent>(event: T): Promise<void> {
    ...
    logger.debug("开始发布事件", { event_type: eventType, event_id: (event as any).event_id, trace_id: traceId });
    ...
    if (handlers.length === 0) {
      logger.debug("事件无处理器，发布完成", { event_type: eventType, event_id: (event as any).event_id });
      return;
    }
    ...
    logger.debug("事件发布完成", { event_type: eventType, event_id: (event as any).event_id, handler_count: handlers.length });
  }
