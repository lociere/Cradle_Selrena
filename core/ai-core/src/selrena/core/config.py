...
class PersonaConfig(BaseModel):
    """
    OC人设核心配置，由TS内核从全局configs/persona.yaml注入，运行时完全冻结。
    人格血肉（性格/外观/风格）完全由 knowledge-base.json persona.* 条目承载；
    此处仅保留最小身份锚定与安全策略。
    """
    model_config = ConfigDict(frozen=True)

    # 基础身份信息（最小锚定：名字用于提示词首行，其余由知识库描述）
    ...
    base: BasePersona
    # 人格模式：控制 PersonaInjector 提示词厚度
    # api / local_base  → 知识库提供所有风格引导 + minimal 提示词锚定
    # local_finetune    → 说话风格已烘焙进权重，跳过人设 KB 检索，极简提示词
    persona_mode: str = 'api'
    safety: SafetyPolicy
...
class InferenceConfig(BaseModel):
    ...
    class EmbeddingConfig(BaseModel):
        enabled: bool                   # 是否启用向量引擎
        model_path: str                 # 嵌入模型本地路径
        device: str                     # 推理设备：cpu / cuda / mps
