...
class DomainEventBus:
    ...
    async def publish(self, event: DomainEvent) -> None:
        ...
        # 包装处理器，捕获单个处理器的异常，写入 DLQ，避免连锁崩溃
        async def wrapped_handler(handler: Callable, e: DomainEvent) -> None:
            try:
                ...
                # v4.5 DLQ: 将失败事件写入死信队列
                try:
                    from selrena.core.observability.dlq_manager import DeadLetterQueue
                    dlq = DeadLetterQueue()
                    await dlq.enqueue(
                        trace_id=e.trace_id,
                        event_type=e.event_type,
                        payload={"event_class": type(e).__name__, "trace_id": e.trace_id},
                        exception=ex,
                    )
                except Exception:
                    pass  # DLQ 自身故障不应影响事件总线
