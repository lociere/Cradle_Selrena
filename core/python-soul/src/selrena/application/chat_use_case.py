...
from selrena.ipc_server.contracts.kernel_ingress_contracts import PerceptionEventContentModel
from selrena.llm_engine.llm_engine import LLMEngine, LLMMessage, LLMRequest
from selrena.llm_engine.multimodal_router import MultimodalRouter, VisionMessage
...
class ChatUseCase(BaseUseCase[ChatInput, ChatOutput]):
    ...
    # v4.5: 上下文水位控制器（可选，未注入时不做截断）
    context_allocator: ContextAllocator | None = field(default=None)

    # 情绪类型 → 中文提示词映射（用于知识库查询增强）
    _EMOTION_HINT_MAP: dict[str, str] = field(default_factory=lambda: {
        "calm": "平静",
        "happy": "开心",
        "shy": "害羞",
        "angry": "生气",
    ...
    def _build_memory_digest(self, user_text: str, multimodal_text: str, emotion_hint: str = "") -> tuple[str, str, str]:
        """构建长期记忆 + 知识上下文。

        返回：(preference_text, ltm_text, knowledge_text)
        人设由 PersonaInjector 独占编译，不经过此处。
        """
        parts = [p for p in [user_text, multimodal_text] if p]
        if emotion_hint:
        ...
        preference_text = "\n".join([f"偏好：{mem.content}" for mem in preference_memories])
        ...
        knowledge_entries = self.self_entity.knowledge_base.get_knowledge(query=memory_query)
        knowledge_text = "\n".join([f"知识：{entry.content}" for entry in knowledge_entries])

        return preference_text, ltm_text, knowledge_text
...
===== 短期记忆 =====
{short_term_digest if short_term_digest else "无"}

===== 长期偏好 =====
{preference_text if preference_text else "无"}

===== 相关记忆 =====
{ltm_text if ltm_text else "无"}
...
""".strip()
    ...
    async def _execute(self, input_data: ChatInput, trace_id: str) -> ChatOutput:
        ...
        async with runtime.lock:
            ...
            # 步骤2：构建长期记忆 / 知识上下文
            # ======================================
            emotion_type = current_emotion.get("emotion_type", "")
            emotion_hint = self._EMOTION_HINT_MAP.get(emotion_type, "")
            preference_text, ltm_text, knowledge_text = self._build_memory_digest(
                user_text=user_text,
            ...
            persona_prompt = self.self_entity.persona_injector.build_persona_prompt(
                ...
                address_mode=input_data.address_mode,
            )
            ...
            # v4.5 Phase 3: 上下文水位控制 — 按 Zone 优先级截断各路素材
            if self.context_allocator is not None:
                allocated = self.context_allocator.allocate(
                    system_persona=persona_prompt,
                ...
                # 使用截断后的素材
                knowledge_text = allocated.knowledge_text
                short_term_digest = allocated.short_term_digest
                _session_summary = allocated.session_summary
                preference_text = allocated.preference_text
                ltm_text = allocated.ltm_text
                multimodal_text = allocated.multimodal_text
                if allocated.was_truncated:
                    logger.info("上下文已水位截断", trace_id=trace_id, total_tokens=allocated.total_estimated_tokens)
            else:
                _session_summary = session.summary_text
            ...
            # ambient 沉默（clean_reply 为空）时不写入会话历史和短期记忆，
            # 避免产生没有对应 assistant 的孤儿 user 轮次污染上下文。
            if clean_reply:
                ...
                logger.debug("会话态与短期记忆沉淠完成", trace_id=trace_id)
            else:
                logger.debug("月见选择沉默，跳过记忆沉淠", trace_id=trace_id)
