...
from selrena.llm_engine.llm_engine import LLMEngine
from selrena.llm_engine.multimodal_router import MultimodalRouter
...
from selrena.application.agent_synthesis_use_case import AgentSynthesisUseCase
from selrena.application.memory_sync_use_case import MemorySyncUseCase
from selrena.application.context_allocator import ContextAllocator
from selrena.memory_pipeline.memory_consolidator import MemoryConsolidator
from selrena.ipc_server.outbound.kernel_bridge import KernelBridge
from selrena.ipc_server.inbound.kernel_ingress_cortex import KernelIngressCortex
from selrena.ipc_server.inbound.kernel_event_adapter import KernelEventInboundAdapter
from selrena.ipc_server.outbound.kernel_event_adapter import KernelEventOutboundAdapter
...
class DIContainer:
    ...
    def init(self, config: GlobalAIConfig) -> None:
        ...
        # 向量嵌入引擎（可选：配置启用且模型可用时才注入知识库）
        embedding_engine = EmbeddingEngine()
        embedding_cfg = getattr(config.inference, "embedding", None)
        if embedding_cfg and embedding_cfg.enabled:
            embedding_engine.load(
                model_path=embedding_cfg.model_path,
                device=embedding_cfg.device,
            )
        self._instances["embedding_engine"] = embedding_engine
        # 注入知识库（semantic_rag 模式备用）
        self_entity.knowledge_base.set_embedding_engine(embedding_engine)
        # 注入长期记忆（主要消费者——对持续增长的记忆做语义检索）
        self_entity.long_term_memory.set_embedding_engine(embedding_engine)

        # ======================================
        # 4. 应用层用例实例
        # ======================================
        # v4.5 Phase 3: 上下文水位控制器
        context_allocator = ContextAllocator(
            max_context_tokens=config.inference.model.max_tokens,
            response_reserve_tokens=512,
        )
        self._instances["context_allocator"] = context_allocator
        ...
        agent_plan_use_case = AgentPlanUseCase(self_entity=self_entity, llm_engine=llm_engine)
        self._instances["agent_plan_use_case"] = agent_plan_use_case

        agent_synthesis_use_case = AgentSynthesisUseCase(self_entity=self_entity, llm_engine=llm_engine)
        self._instances["agent_synthesis_use_case"] = agent_synthesis_use_case

        # v4.5 Phase 4: 记忆固化引擎
        memory_consolidator = MemoryConsolidator(
            llm_engine=llm_engine,
            long_term_memory=self_entity.long_term_memory,
            embedding_engine=embedding_engine if embedding_engine.is_available() else None,
        )
        self._instances["memory_consolidator"] = memory_consolidator
        ...
        inbound_adapter = KernelEventInboundAdapter(
            ...
            agent_synthesis_use_case=agent_synthesis_use_case,
        )
        ...
        kernel_bridge.register_handler(
            "agent_synthesis",
            lambda msg: inbound_adapter.on_agent_synthesis(ingress_cortex.parse_agent_synthesis(msg)),
        )
        ...
        kernel_bridge.register_handler(
            "consolidate_memory",
            lambda msg: self._handle_consolidate_memory(msg, memory_consolidator, self_entity),
        )
    ...
    async def _handle_consolidate_memory(
        self, msg: dict, consolidator: MemoryConsolidator, self_entity: SelrenaSelfEntity,
    ) -> dict:
        """v4.5: 处理记忆固化请求。"""
        scene_id = msg.get("payload", {}).get("scene_id", "")
        reason = msg.get("payload", {}).get("reason", "idle_timeout")
        trace_id = msg.get("trace_id", "")
        if not scene_id:
            return {"consolidated_count": 0, "trace_id": trace_id}
        short_term_memory = self_entity.get_short_term_memory(scene_id)
        count = await consolidator.consolidate(
            scene_id=scene_id,
            short_term_memory=short_term_memory,
            reason=reason,
        )
        return {"consolidated_count": count, "trace_id": trace_id}
