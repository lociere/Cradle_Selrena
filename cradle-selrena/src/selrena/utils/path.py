...
from selrena.utils.env import IS_PRODUCTION
...
def _get_project_root() -> Path:
    ...
    # guard: if module is imported from site-packages (installed package)
    # or the path itself ends with src/selrena we don't want to treat that as
    # the project root, because creating assets/configs/data there is wrong.
    if 'site-packages' in str(current_path) or str(current_path).endswith(os.path.join('src','selrena')):
        # fall back to current working directory or one level up
        cwd = Path.cwd()
        if (cwd / 'pyproject.toml').exists() or (cwd / 'README.md').exists():
            return cwd
        # otherwise use parent of cwd to be safe
        return cwd.parent
...
# -------------------------- 初始化执行 --------------------------
# 在导入时确保目录存在
# 如果包安装在 site-packages 中，则不要在包目录创建全局文件夹
if 'site-packages' not in str(ProjectPath.PROJECT_ROOT):
