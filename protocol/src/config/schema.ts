 ...
 * 两大配置域：
 *   system  — 系统级配置（端口号 / IPC / 日志级别 / 生命周期 / 插件沙箱）
 *   persona — 角色人格与 AI 层配置（人格 / 推理 / LLM 提供商）
 *
 * 约定：
...
// ═══════════════════════════════════════════════════════════════
//  system — 系统级配置（合并原 app + kernel 两域）
// ═══════════════════════════════════════════════════════════════

/** 应用元信息字段（嵌入 SystemConfig 的顶层） */
...
/** 角色基础档案（最小身份锚定，性格/外观/风格等血肉完全由知识库承载） */
...
/**
 * 完整人格配置
 * - 身份锚定：base（name / nickname）
 * - 人格血肉：由 knowledge-base.json persona.* 条目承载
 * - 提示词行为：persona_mode 控制 PersonaInjector 厚度
 * - 安全红线：safety（提示词注入 + 输出校验）
 */
export const PersonaConfigSchema = z.object({
  base: BasePersonaSchema,
  /**
   * 人格模式：
   * - api / local_base  → 知识库提供所有风格引导
   * - local_finetune    → 风格已烘焙进权重，跳过人设 KB 检索
   */
  persona_mode: z.enum(["api", "local_base", "local_finetune"]).default("api"),
...
export const EmbeddingConfigSchema = z.object({
  /** 是否启用向量引擎 */
  enabled: z.boolean().default(false),
  /** sentence-transformers 模型本地路径 */
  model_path: z.string().default("assets/models/m3e-small"),
  /** 推理设备：cpu / cuda / mps */
...
/**
 * system.yaml — 系统级统一配置
 * 合并原 app.yaml（应用元信息）+ kernel.yaml（内核运行时）
 */
export const SystemConfigSchema = AppConfigSchema.merge(KernelConfigSchema);
...
/** 知识条目范围 */
export const KnowledgeScopeSchema = z.enum(["persona", "knowledge"]);
...
export const KnowledgeEntrySchema = z.object({
  ...
  /** 编译分组（仅 scope=persona 有效）：identity / style / example / trait / emotion:<type> / context:<mode> / safety */
  compile_group: z.string().default(""),
  /** 知识内容文本 */
...
export const KnowledgeRetrievalConfigSchema = z.object({
  ...
  mode: z.enum(["full_injection", "semantic_rag"]).default("full_injection"),
