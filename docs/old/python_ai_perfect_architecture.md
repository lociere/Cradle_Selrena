...
- **AgentPort**: MCP 指令生成接口（与 Agent 模块配合）

**说明**: AgentPort 是为了支持 MCP 协议，允许应用层生成标准化指令而不直接与硬件/网络交互。
...
- **engine_pool/BrainRouter**: 引擎池与路由器，负责策略调度、容灾和多模态转接（视觉->文本）
