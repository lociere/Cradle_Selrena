...
> 🚀 本指南是针对 `cradle-selrena` 仓库中 `src/selrena` 包的全层级开发说明。遵循 DDD 分层架构，保持各层职责单一。文档中包含 Agent/MCP 指令生成和容器注册等内容，开发前建议先浏览 [全局完美架构文档](../Cradle-Selrena：全局完美架构（TS-Rust 无缝升级）.md) 以了解整体设计。

## 目录
1. [核心基础设施层 (core)](#1-核心基础设施层core)
2. [适配器层 (adapters)](#2-适配器层adapters)
3. [领域层 (domain)](#3-领域层domain)
4. [数据模型层 (schemas)](#4-数据模型层schemas)
5. [应用服务层 (application)](#5-应用服务层application)
6. [工具层 (utils)](#6-工具层utils)
7. [业务核心层（persona/memory/intention/agent）](#7-业务核心层personamemoryintentionagent)
8. [推理引擎层 (inference)](#8-推理引擎层inference)
...
> **说明**: agent 文件夹用于 MCP 指令生成，与内核协同工作，详见第7节。




## 1. 核心基础设施层（core）
这一层负责支撑整个平台的运行环境，包含配置管理、事件总线客户端等纯基础组件，
所有模块通过生命周期接口注册。名称保持 "core"，与全局架构一致。

> **注意**：启动逻辑（`ai_service.py`/`main_service.py`）已迁出 core，
> 现在置于 `service/` 包中，它们属于运行时 glue，而非基础设施。  
...
### 开发要点
1. **生命周期**: 继承 `Lifecycle` 接口（`core/lifecycle.py`），在 `on_start`/`on_stop`里启动/清理资源。所有实现应在 `container.py` 的 `register()` 方法中通过容器注册为单例。
...
from selrena.core.logger import get_logger  # logger now lives in core

class MyService(Lifecycle):
    async def on_start(self):
        cfg = global_config.get_system().my_service
...
```python
# event_bus_client example (internal API)
from selrena._internal.core.event_bus import EventBusClient

bus = EventBusClient()
await bus.connect()
await bus.register_handler("user.input", self.handle_input)
```
...
- `path.py`：统一使用 `pathlib.Path`，提供项目根路径常量和各类资源目录，借鉴旧架构的 ProjectPath 设计。**改进版会自动选择最高级的工作区目录作为根，避免包内副本产生多余目录。**
- `env.py`：环境检测/解析（生产/开发模式、OS 判断、类型安全的 env getter）。
- `dicts.py`：字典路径读写、合并和默认填充工具，方便配置处理。
- `event_payload.py`：深度提取事件字段、解析复杂载荷。
- `string.py`：文本清洗与 JSON 提取，适用于 LLM 输出；直接借鉴自旧架构并修复中文括号处理。
...
- **agent**: 仅负责生成 MCP 指令，执行由内核完成。Agent 层依赖 `AgentPort`，可通过 `container` 注册并提供给服务使用。当前示例用于工具调用和多步计划。
...
# agent/command_generator.py
class CommandGenerator:
    def __init__(self, port: AgentPort):
        self._port = port

    async def build(self, intent, tools):
        # 根据意图生成指令结构
        cmd = {...}
        await self._port.send_command(cmd)
```
...
### 添加新引擎
1. 在 `inference/engine_pool/` 下创建子模块（`myengine.py`）。
2. 继承 `BaseBrainBackend` 并实现 `generate`（可选 `initialize`/`cleanup`）。
3. 在配置中注册引擎名称并在 BrainRouter 中使用。
4. 编写单元测试模拟输入→输出流程。

### 调度器 & 路由
* `inference/engine_pool/router.py` 是 BrainRouter 实现，用于根据策略选择引擎实例。
* 引擎调度应保持纯逻辑，负载信息通过参数传入。
