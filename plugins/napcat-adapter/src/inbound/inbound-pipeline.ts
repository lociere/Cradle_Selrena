...
export class InboundPipeline {
  ...
  async process(event: OB11MessageEvent): Promise<void> {
    ...
    // ── 通过门控后：执行耗时操作并构建感知事件 ────────────────────
    // 含唤醒词或符合 always/chat 策略时延续/刷新焦点计时器；
    // 焦点期内普通消息（wake_word_focus_with_timeout + currentlyFocused）不重置计时器。
    const shouldExtendFocus =
      sourceFocusPolicy === 'always_focused' ||
      sourceFocusPolicy === 'chat_or_wake_focus_with_timeout' ||
      containsWakeWord;

    if (shouldExtendFocus) {
